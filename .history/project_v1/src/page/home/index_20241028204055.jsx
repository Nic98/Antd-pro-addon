...
export default class HomeComponent extends Component{
    ...
    async componentDidMount() {
      ...
      this.setState({
        ...
        console.log('lowcodePages:', this.state.lowcodePages);
      })
    }
              ...
              <Sider width={200} style={{ background: '#fff' }}>
                ...
                >
                  ...
                  <SubMenu key="sub7" title={<span><Icon type="shopping-cart" />{'低代码'}</span>}>
                    {lowcodePages.map((item) => {
                      const url = `http://localhost:5556/preview.html?scenarioName=${item.pageName}`;
                      return (
                        <Menu.Item key={item.pageId} onClick={() => { this.bindCurrentKey(`/${item.pageName}`) }}>{item.pageName}</Menu.Item> 
                     )
                    })}
                  </SubMenu>
                  http://localhost:5556/preview.html?scenarioName=test1
                  {<iframe src={`http://localhost:5556/preview.html?scenarioName=${currentKey}`} style={{ width: '100%', height: '100vh', border: 'none' }} />}
                </Menu>
              </Sider>
