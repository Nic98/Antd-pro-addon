...
export type ValidateErrorGroup = Record<string, {
    errors: unknown[];
}>;
...
export type NormalizedFieldOption = RequiredSome<FieldOption, 'parseName' | 'forceUpdate' | 'first' | 'onChange' | 'autoUnmount' | 'autoValidate'>;
export type Rule = {
    /**
...
export type InitOption<ValueType = any, ValueName extends string = 'value', Trigger extends string = 'onChange', Props = object> = {
    /**
     * 唯一标识
     */
    id?: string;
...
export interface NormalizedFieldMeta extends Omit<RequiredSome<FieldMeta, 'name' | 'state' | 'valueName' | 'initValue' | 'disabled'>, 'rules'> {
    rules: Rule[];
}
